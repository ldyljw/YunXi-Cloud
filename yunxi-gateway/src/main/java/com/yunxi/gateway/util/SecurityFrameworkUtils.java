...
import com.yunxi.gateway.filter.security.LoginUser;
 ...
 * <p>
 * 安全服务工具类
 * 复制 yunxi-spring-boot-starter-security 的 SecurityFrameworkUtils 类
 * </p>
 *
 * @author lidy
 * @since 2024-04-21
...
public class SecurityFrameworkUtils {
    ...
    public static void setLoginUserHeader(ServerHttpRequest.Builder builder, LoginUser user) {
        try {
            ...
            userStr = URLEncoder.encode(userStr, StandardCharsets.UTF_8.name());// 编码，避免中文出现乱码
            builder.header(LOGIN_USER_HEADER, userStr);
        } catch (Exception e) {
            log.error("[setLoginUserHeader][序列化 user({}) 发生异常]", user, e);
            throw e;
        }
    }
