...
import com.yunxi.framework.pay.core.enums.order.PayOrderDisplayModeEnum;
...
import static com.yunxi.framework.pay.core.enums.order.PayOrderStatusRespEnum.CLOSED;
import static com.yunxi.framework.pay.core.enums.order.PayOrderStatusRespEnum.WAITING;
import static com.yunxi.framework.test.core.util.RandomUtils.*;
...
public class AlipayPcPayClientTest extends AbstractAlipayClientTest {
    ...
    public void testUnifiedOrder_urlSuccess() throws AlipayApiException {
        ...
        when(defaultAlipayClient.pageExecute(argThat((ArgumentMatcher<AlipayTradePagePayRequest>) request -> true), eq(Method.GET.name()))).thenReturn(response);
        // 准备请求参数
    ...
    public void testUnifiedOrder_formSuccess() throws AlipayApiException {
        ...
        String notifyUrl = randomURL();
        AlipayTradePagePayResponse response = randomPojo(AlipayTradePagePayResponse.class, o -> o.setSubCode(""));
        when(defaultAlipayClient.pageExecute(argThat((ArgumentMatcher<AlipayTradePagePayRequest>) request -> true), eq(Method.POST.name()))).thenReturn(response);
        // 准备请求参数
        String outTradeNo = randomString();
        Integer price = randomInteger();
        PayOrderUnifiedReqDTO reqDTO = buildOrderUnifiedReqDTO(notifyUrl, outTradeNo, price);
